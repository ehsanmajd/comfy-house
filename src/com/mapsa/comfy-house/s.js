...
class Product{
    ...
    render(){
      ...
      const button = builder.create('button').className('bag-btn').appendTo(div).on("click", ()=>{
...
class Cart {  
  render(){
    ...
    cartItems.forEach(cartItem => {
      ...
      builder.create('i').className('fas fa-chevron-up').appendTo(div2).onclick(()=>{
          this.inc(cartItem)
      })
      builder.create('p').className('item-amount').text(cartItem.quantity).appendTo(div2)
      builder.create('i').className('fas fa-chevron-down').appendTo(div2).onclick(()=>{
          this.dec(cartItem)
      })
      })
    }
